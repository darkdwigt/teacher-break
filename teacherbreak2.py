...
def main():
    exam_duration = int(input("Length of exam in minutes: "))
    learners = int(input("Number of learners: "))
    num_teachers = int(input("Number of teachers: "))

    teacher_names = [input(f"Enter the name of teacher {i+1}: ") for i in range(num_teachers)]

    required_teachers = math.ceil(learners / 30.0)
    
    if num_teachers <= required_teachers:
        print(f"All {num_teachers} teachers are needed on duty for the entire duration due to {learners} learners.")
        return

    schedule = schedule_breaks(learners, exam_duration, teacher_names)

    for start_time, end_time, on_duty, break_set in schedule:
        print(f"From minute {start_time:.2f} to {end_time:.2f}, {on_duty} teachers are on duty. Teachers on break: {', '.join(break_set) or 'None'}.")
    ...
    for teacher, duration in total_breaks.items():
        print(f"{teacher}: {duration:.2f} minutes")
